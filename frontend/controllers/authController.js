...
export const register = async(req, res) =>{
    ...
    try{
        ...
        res
            .status(200)
            .json({success:true, message:'User sucessfuly'})

    }catch(error){
        res
            .status(404)
            .json({success:false, message:'User not sucessfuly'})
    }
}
...
export const login = async(req, res) => {
    ...
    try{
        ...
        if(!isPasswordMatch){
            return res.status(400).json({status:false, message:"There is no User"});
        }
